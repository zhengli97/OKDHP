...
class HourglassNet(nn.Module):
    ...
    def __init__(self, block, cfg, num_kernel=3):
        ...
        # Shared Layer
        for i in range(self.num_stacks - self.individual_number):
            setattr(self, 'ind_s_' + str(i),
                    self._make_individual(block, num_blocks, self.num_feats, ch))
            setattr(self, 'score_s_' + str(i),
                    nn.Conv2d(ch, num_classes, kernel_size=1, bias=True))

            setattr(self, '_fc_s_' + str(i),
                    nn.Conv2d(ch, ch, kernel_size=1, bias=True))
            setattr(self, '_score_s_' + str(i),
                    nn.Conv2d(num_classes, ch, kernel_size=1, bias=True))

        # Do Not Shared
        for branch_idx in range(self.num_branches):
            for indiv_idx in range(self.individual_number):
                setattr(self, 'ind_b_' + str(branch_idx) + '_s_' + str(indiv_idx),
                        self._make_individual(block, num_blocks, self.num_feats, ch))
                setattr(self, 'score_b_' + str(branch_idx) + '_s_' + str(indiv_idx),
                        nn.Conv2d(ch, num_classes, kernel_size=1, bias=True))

                if indiv_idx < self.individual_number - 1:
                    setattr(self, '_fc_b_' + str(branch_idx) + '_s_' + str(indiv_idx),
                            nn.Conv2d(ch, ch, kernel_size=1, bias=True))
                    setattr(self, '_score_b_' + str(branch_idx) + '_s_' + str(indiv_idx),
                            nn.Conv2d(num_classes, ch, kernel_size=1, bias=True))
    ...
    def forward(self, x):
        ...
        for stack_idx in range(self.num_stacks - self.individual_number):
            y = getattr(self, 'ind_s_' + str(stack_idx))(x)
            score = getattr(self, 'score_s_' + str(stack_idx))(y)
            out.append(score)

            fc_ = getattr(self, '_fc_s_' + str(stack_idx))(y)
            score_ = getattr(self, '_score_s_' + str(stack_idx))(score)
            x = x + fc_ + score_
        ...
        for branch_idx in range(self.num_branches):
            ...
            for indiv_idx in range(self.individual_number):
                y = getattr(self, 'ind_b_' + str(branch_idx) + '_s_' + str(indiv_idx))(vars()['x' + str(branch_idx)])
                score = getattr(self, 'score_b_' + str(branch_idx) + '_s_' + str(indiv_idx))(y)
                # i+=1
                out.append(score)
                # score branch_1, 2, 3,...
                if indiv_idx < self.individual_number - 1:
                    fc_ = getattr(self, '_fc_b_' + str(branch_idx) + '_s_' + str(indiv_idx))(y)
                    score_ = getattr(self, '_score_b_' + str(branch_idx) + '_s_' + str(indiv_idx))(score)
